...
The current equivalents are [weights.py](../lyaforecast/lyaforecast/weights.py),
lines 135–156, 184–206, 255–273, and 339–358. The present forest auto-power takes
the final coefficient elements in
[covariance.py](../lyaforecast/lyaforecast/covariance.py), lines 335–365.
...
The current [weight update](../lyaforecast/lyaforecast/weights.py), lines 149–154,
still has the aliasing comment followed by `signal_power = self._p3d_w`.
Tracing [power_spectrum.py](../lyaforecast/lyaforecast/power_spectrum.py), lines
109–183, shows that `_p3d_w` contains intrinsic clustering and instrumental
smoothing, not sampling aliasing. The [final covariance](../lyaforecast/lyaforecast/covariance.py),
...
`method="legacy"` path: see [kernels/weights.py](fishhighz/kernels/weights.py),
lines 9–37, and [weights.py](fishhighz/weights.py). Its agreement with lyaforecast
