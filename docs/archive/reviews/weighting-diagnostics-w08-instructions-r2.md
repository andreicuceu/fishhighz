...
Status: revision 1 is implemented and independently reviewed with one required
representability correction. Ordinary analytic, noise, survey and saved-reference
checks pass, but positive `l_p*v` can underflow to zero without rejection while
the final coefficients remain representable. Revision 2 is proposed for user
approval and implementation-agent dispatch. The selected fixed-reference option,
cumulative compatibility, profile defaults, package Step 13 and forecast
acceptance are unchanged. This plan does not authorize implementation dispatch.
Created: 2026-09-16.
...
is archived. Also read the W08 r1 handoff (local-only path: `reviews/weighting-diagnostics-w08-r1.md`),
the W08 r1 review (local-only path: `reviews/weighting-diagnostics-w08-review-r1.md`), and the
exact r1 assignment (local-only path: `reviews/weighting-diagnostics-w08-instructions-r1.md`).
