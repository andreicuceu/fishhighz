# Forest weighting W08, revision 1: explicit fixed inverse-variance option

Status: proposed for user approval and implementation-agent dispatch. W07 passes
independent scientific review. On 2026-09-16 the user selected planning the
fixed-reference option, preserving cumulative weighting for compatibility.
This authorizes this plan, not implementation dispatch, a profile-default change
or scientific acceptance of the package forecasts. Package Step 13 is unchanged.
Created: 2026-09-16.
...
sequence. It replaces no estimator automatically. The new explicit method uses
the existing fixed ForestWeights and noise APIs; no new public class, mode axis,
provider protocol, survey configuration layer or production optimizer is needed.

## 2. Read and verify live contracts

Read workspace/package AGENTS.md, design section 0, main roadmap handover/
progress register, the diagnostic roadmap (local-only path: `../../FISHHIGHZ_WEIGHTING_DIAGNOSTICS_PLAN.md`),
W07 report (local-only path: `reviews/weighting-diagnostics-w07-r1.md`),
W07 review (local-only path: `reviews/weighting-diagnostics-w07-review-r1.md`), and the reference
conventions in W05 (local-only path: `reviews/weighting-diagnostics-w05-r1.md`) and
W06 (local-only path: `reviews/weighting-diagnostics-w06-r1.md`). The
reviewed W07 assignment (local-only path: `reviews/weighting-diagnostics-w07-instructions-reviewed-r1.md`)
is archived. `IMPLEMENTATION_STEP.md` remains the separate package Step 13.
...
Add only this method to the existing keyword interface:
...
Expected production edit: `fishhighz/weights.py`; a small additional numeric
helper in `fishhighz/kernels/weights.py` is allowed if necessary. Preserve
...
Add a focused `tests/test_inverse_variance_weights.py` and minimally extend
existing weight/survey tests where useful. Update the README's weight-method
section with the new direct-scalar option, response ownership, fixed-fiducial
semantics and its conditional optimality claim. Do not change profile recipes,
legacy adapters, CLI/INI defaults, public result classes or existing examples
except a minimal relevant documentation snippet. Compatibility remains legacy.
...
Write `reviews/weighting-diagnostics-w08-r1.md`. Any small saved-input check
script/results go in a new `.validation/forest-weight-diagnostics/w08-r1-<UTC>/`.
Lead with what the new option computes and what the focused checks establish.
...
`reviews/weighting-diagnostics-w08-review-r1.md`, beginning with **Scientific
