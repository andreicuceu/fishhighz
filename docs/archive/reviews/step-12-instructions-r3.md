...
Revision: 3 (2026-09-14; user narrowed real validation to 15x2pt).
State: revision-2 review requires convergence-evidence repair (R2); the forest
...
roadmap handover (local-only path: `../../FISHHIGHZ_IMPLEMENTATION_PLAN.md`), the
revision-2 implementation (local-only path: `reviews/step-12.md`), and
independent review (local-only path: `reviews/step-12-review-r2.md`). Implement this step only when
...
## Revision 3 repair scope and remaining scientific decision
...
The original five R1 payload probes now reject. New review passed 1059 quick
...
3. Reproduce R2 using self-contained tiny real-shaped evidence and a copied
   ...
   successes. Retain all original five R1 regressions and existing 1059 tests.
...
self-contained examples may diagnose the cumulative update's fixed points,
...
Revision 3 is explicitly restricted to 15x2pt, using its 15 individual-spectrum
...
Revision 3 authorizes only the bounded 15x2pt work specified above. Ordinary quick tests remain
...
production CLI/serialization, new optimization algorithms, compilation and Step
13. Preserve the existing recipe library; execute only its 15x2pt recipe and
...
It is allowed to instrument a validation-owned wrapper/subclass or call the
read-only reference to capture upstream arrays. Do not edit the sibling checkout.
...
require relative agreement <=1e-6 with the fresh legacy calculation. Report
...
Use the existing prepare_bin/run_bin/run_forecast and KaiserModel path. “Maximum
...
Preserve all 1059 existing tests and their numerical contracts. Add focused
