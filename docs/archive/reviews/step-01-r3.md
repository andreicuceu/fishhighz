...
The retained validation driver (local-only path: `../.validation/validate.py`) records every exact
...
import probe (local-only path: `../.validation/probe.py`) using `python -I -c` from a fresh `/tmp`
...
See versions (local-only path: `../.validation/versions.log`),
Python details (local-only path: `../.validation/python-version.log`), and build logs for provenance.
...
Final driver results (local-only path: `../.validation/validation.log`) record all checks exiting 0.
