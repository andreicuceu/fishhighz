...
Per-bin values, individual-spectrum effects, exact tested interactions, rank, correlation and ellipse-area changes are retained in attribution-summary.json (local-only path: `../.validation/s4/attribution-summary.json`). Source definitions and unchanged assumptions are in [s4-profile-inventory.md](s4-profile-inventory.md).
